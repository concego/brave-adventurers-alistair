# Enemy.gd — Goblin corpo a corpo
# pit_awareness=0.7: para na maioria dos fossos, às vezes cai

extends "res://scripts/EnemyBase.gd"

@export var attack_damage: float   = 10.0
@export var attack_range: float    = 60.0
@export var flee_hp_threshold: float = 0.25

var state: String       = "patrol"
var patrol_dir: float   = 1.0
var patrol_timer: float = 0.0
var attack_cooldown: float = 0.0

const FRAME_W: int = 80
const FRAME_H: int = 96
const ANIM_ROW    = {"idle":0,"walk":1,"attack":2,"hit":3,"death":4}
const ANIM_FRAMES = {"idle":2,"walk":4,"attack":3,"hit":2,"death":4}
const ANIM_FPS    = {"idle":6,"walk":8,"attack":12,"hit":8,"death":5}

var _cur_anim: String = "idle"
var _cur_frame: int   = 0

@onready var sprite: Sprite2D  = $Sprite2D
@onready var anim_timer: Timer = $AnimTimer

func _ready() -> void:
	pit_awareness = 0.7
	hp            = max_hp
	_ready_base()
	if anim_timer:
		anim_timer.timeout.connect(_advance_frame)
		anim_timer.start(1.0 / ANIM_FPS["idle"])

func _advance_frame() -> void:
	_cur_frame = (_cur_frame + 1) % ANIM_FRAMES.get(_cur_anim, 1)
	if sprite and sprite.texture:
		sprite.region_enabled = true
		sprite.region_rect    = Rect2(_cur_frame * FRAME_W, ANIM_ROW.get(_cur_anim,0) * FRAME_H, FRAME_W, FRAME_H)
		sprite.flip_h         = facing_right

func _play_anim(anim: String) -> void:
	if _cur_anim == anim: return
	_cur_anim  = anim
	_cur_frame = 0
	if anim_timer:
		anim_timer.wait_time = 1.0 / ANIM_FPS.get(anim, 6)
		anim_timer.start()

func _physics_process(delta: float) -> void:
	if _process_base(delta): return

	attack_cooldown = max(attack_cooldown - delta, 0.0)

	match state:
		"patrol": _patrol(delta)
		"chase":  _chase()
		"attack": _do_attack()
		"flee":   _flee()

	move_and_slide()

func _patrol(delta: float) -> void:
	patrol_timer -= delta
	if patrol_timer <= 0:
		patrol_dir   *= -1
		patrol_timer  = randf_range(1.5, 3.0)
	facing_right  = patrol_dir > 0
	velocity.x    = _safe_move_toward(global_position + Vector2(patrol_dir * 100, 0), speed * 0.5)
	_play_anim("walk")
	if player and global_position.distance_to(player.global_position) < detection_range:
		state = "chase"

func _chase() -> void:
	if not player: state = "patrol"; return
	var dist = global_position.distance_to(player.global_position)
	if dist <= attack_range:   state = "attack"; return
	if dist > detection_range * 1.5: state = "patrol"; return
	velocity.x = _safe_move_toward(player.global_position, speed)
	_play_anim("walk")

func _do_attack() -> void:
	if not player: state = "patrol"; return
	if global_position.distance_to(player.global_position) > attack_range:
		state = "chase"; return
	velocity.x = 0
	if attack_cooldown > 0: _play_anim("idle"); return
	attack_cooldown = 1.4
	if player.has_method("open_parry_window"):
		player.open_parry_window()
	_play_anim("attack")
	get_tree().create_timer(0.4).timeout.connect(func():
		if player and global_position.distance_to(player.global_position) <= attack_range * 1.2:
			player.take_damage(attack_damage)
	)

func _flee() -> void:
	if not player: state = "patrol"; return
	velocity.x = _safe_move_toward(global_position - (player.global_position - global_position), speed * 1.3)
	_play_anim("walk")

func take_damage(amount: float) -> void:
	hp -= amount
	_play_anim("hit")
	if hp <= 0: _die(); return
	if hp / max_hp <= flee_hp_threshold:
		state = "flee"

# Sobrescreve reação ao fosso: para na borda
func _pit_reaction(_dir: float, _spd: float) -> float:
	return 0.0
